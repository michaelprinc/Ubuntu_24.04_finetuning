...
tee /media/michael-princ/E85AE8F65AE8C284/Data_science_projects/Ubuntu_24.04_fine_tuning/scripts/dev_env.sh > /dev/null << 'EOF'
#!/bin/bash
...
case "$1" in
    ...
    "workspace")
        if [ -z "$2" ]; then
            echo "Usage: $0 workspace <workspace_name>"
            exit 1
        fi
        mkdir -p "$HOME/Workspaces/$2"
        cd "$HOME/Workspaces/$2"
        code .
        ;;
    *)
        echo "Usage: $0 {status|update|workspace}"
        echo "  status    - Show development environment status"
        echo "  update    - Update all development tools"
        echo "  workspace - Create and open a new workspace"
        exit 1
        ;;
esac
EOF

chmod +x /media/michael-princ/E85AE8F65AE8C284/Data_science_projects/Ubuntu_24.04_fine_tuning/scripts/dev_env.sh
